...
{
    $block = $html->find("div.site-menu__list")[0];
    $newItems = $block->find("div.news-item.news-item--normal");
    ...
    if (isset($newItems) && count($newItems) > 1) {
        for ($i = 1; $i < count($newItems); $i++) {
            ...
            //$id++;
            $title = $newItem->find(".news-item__title", 0)->title;

            $img = $newItem->find(".dt-thumbnail > img", 0)->src;

            $href = 'http://dantri.com.vn' . $newItem->find("a", 0)->href;

            $noidung = $newItem->find(".news-item__content", 0)->innerText();
...
{
    $html = file_get_html($link);
    foreach ($html->find('.dt-news__time') as $element) {
